...
def load_image_to_ddr():
    batch = []
    for idx  in range(len(IAMGE_NAMES_F)):
        pic_name = IMG_DIR + IAMGE_NAMES_F[idx]
        batch.append(Image.open(pic_name).convert('RGBA'))
        if ((idx+1)%BATCH_SIZE == 0):
            IMAGE_BUFF.append(batch)
            batch = []
        

def stitch(batch, image):
    image[0] = np.array(batch[0].resize((320, 160)))
    image[1] = np.array(batch[1].resize((320, 160)))
    image[2] = np.array(batch[2].resize((320, 160)))
    image[3] = np.array(batch[3].resize((320, 160)))
